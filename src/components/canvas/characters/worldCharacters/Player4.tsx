...
//   const { scene: amyScene } = useGLTF('/models/characters/AmyClone.glb') as unknown as GLTFResult
  ...
  const { nodes: mutantNodes, materials: mutantMaterials, animations: mutantAnimations } = useGLTF(`${process.env.NEXT_PUBLIC_API_URL}/file/downloadCharacters?fileName=player4/Mutant.glb`)
  const { actions: mutantActions } = useAnimations(mutantAnimations, mutantGroupRef)

  const { nodes: louiseNodes, materials: louiseMaterials, animations: louiseAnimations } = useGLTF(`${process.env.NEXT_PUBLIC_API_URL}/file/downloadCharacters?fileName=player4/Louise.glb`)
