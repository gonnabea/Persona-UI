...
const Player2Character = () => {
  ...
  const {
    ...
    scene: amyScene,
  } = useGLTF(`models/characters/player2/Amy.glb`)
  ...
  } = useGLTF(`models/characters/player2/Mutant.glb`)
  ...
  return (
    <>
      {/* amy */}
      {character === 'amy' && otherUsers && otherUsers[0] ? (
        <group
          ref={amyGroupRef}
          scale={0.01}
          position={
            otherUsers && otherUsers[0]
              ? [otherUsers[0]?.positionX, otherUsers[0]?.positionY, otherUsers[0]?.positionZ]
              : null
          }
