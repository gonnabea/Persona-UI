...
function TrainingItem2() {
  ...
  //   useEffect(() => {
  //     console.log(items.training_item_2)
  //     console.log(glb.scene)
  //     if (items.training_item_2.installing === true) {
  //       setSelectedItem(glb.scene)
  //     }
  //   }, [items.training_item_2])

  // 가구 설치중일 시 투명하게 보이게 하는 로직.
  //   useEffect(() => {
  //     const installingModel = items.training_item_2.find((training_item_2) => training_item_2.installing === true)
  //     const installingModelIndex = items.training_item_2.findIndex((training_item_2) => training_item_2.installing === true)
  //     if (installingModel) {
  ...
  return (
    <>
      {clonedArr.map((training_item_2_scene, index) => {
        if (items.training_item_2[index].installing === true || items.training_item_2[index].installed === true) {
          return (
            <>
              <Suspense key={index} fallback={null}>
                <primitive
                  ...
                  onContextMenu={(e) => {
                    ...
                    if (isEditMode) {
                      // e.stopPropagation()
                      items.training_item_2[index].installed = false
                      items.training_item_2[index].installing = false
                      forceUpdate(updateIndex + 1)
                    }
                  }}
                  ...
                  rotation={items.training_item_2[index].rotation}
                  object={training_item_2_scene}
                />
              </Suspense>
              {/*  */}(
